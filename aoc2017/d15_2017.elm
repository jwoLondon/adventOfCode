...
part1 input =
    let
        ( startA, startB ) =
            parse input
    in
    compare 40000000 genA genB startA startB 0
...
part2 input =
    let
        ( startA, startB ) =
            parse input
    in
    compare 5000000 (gen2 genA 4) (gen2 genB 8) startA startB 0


compare : Int -> Generator -> Generator -> Int -> Int -> Int -> Int
compare n gA gB prevA prevB total =
    if n < 1 then
        total
        ...
        let
            ...
            score =
                if matchLowBits resultA resultB then
                    1

                else
                    0
        in
        compare (n - 1) gA gB resultA resultB (total + score)


genA : Generator
genA prev =
    modBy 2147483647 (prev * 16807)


genB : Generator
genB prev =
    modBy 2147483647 (prev * 48271)


gen2 : Generator -> Int -> Generator
gen2 generator multiple prev =
    let
        res =
            generator prev
    in
    if modBy multiple res == 0 then
        res

    else
        gen2 generator multiple res


matchLowBits : Int -> Int -> Bool
matchLowBits num1 num2 =
    Bitwise.and 0xFFFF num1 == Bitwise.and 0xFFFF num2


parse : List String -> ( Int, Int )
parse input =
    let
        parseLine =
            matches "(\\d+)"
