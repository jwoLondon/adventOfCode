{- You come across some programs that are trying to implement a software emulation
    ...
    very strong, but you decide to help them anyway. You make a mental note to remind
    ...
    * Increase the skip size by one.
    ...
    beyond the end of the list, the operation reverses using as many extra elements
    as it needs from the front of the list. If the current position moves past the
    end of the list, it wraps around to the front. Lengths larger than the size of
...
multiKnot : Int -> Int -> OffsetList -> TwistLengths -> OffsetList
multiKnot repeats skipSize ( pos, xs ) twistLengths =
    if repeats <= 0 then
        ( pos, xs )
    else
        multiKnot (repeats - 1)
            (skipSize + List.length twistLengths)
            (knot skipSize ( pos, xs ) twistLengths)
            twistLengths


knot : Int -> OffsetList -> TwistLengths -> OffsetList
knot skipSize ( pos, xs ) twistLengths =
    case twistLengths of
        [] ->
            ( pos, xs )
        ...
        twistLength :: tl ->
            ...
            in
            knot (skipSize + 1)
                ( newPos, (reversePart twistLength >> rotate (twistLength + skipSize)) xs )
                tl
        ...
        let
            blockHex =
                sparseHash
                    ...
                    |> toHex
        in
        knotHash (denseHash ++ blockHex) (List.drop 16 sparseHash)
