{- Diagnostics indicate that the local grid computing cluster has been contaminated
   ...
   two-dimensional grid of compute nodes. Each node is either clean or infected
   by the virus.
...
type alias BurstFunction =
    ( Nodes, Carrier, Int ) -> ( Nodes, Carrier, Int )


part1 : List String -> Int
part1 input =
    doBursts burst ( initialNodes input, ( North, ( 0, 0 ) ), 0 ) 10000


part2 : List String -> Int
part2 input =
    doBursts burstPart2 ( initialNodes input, ( North, ( 0, 0 ) ), 0 ) 10000000
...
doBursts : BurstFunction -> ( Nodes, Carrier, Int ) -> Int -> Int
...
burst : BurstFunction
burst ( nodes, ( dir, location ), numInfected ) =
    if Dict.get location nodes == Just Infected then
        ( Dict.insert location Clean nodes
...
burstPart2 : BurstFunction
burstPart2 ( nodes, ( dir, location ), numInfections ) =
    case Dict.get location nodes of
        Just Weakened ->
            ( Dict.insert location Infected nodes, ( dir, location ) |> move, numInfections + 1 )

        Just Infected ->
            ( Dict.insert location Flagged nodes, turnRight ( dir, location ) |> move, numInfections )

        Just Flagged ->
            ( Dict.insert location Clean nodes, reverse ( dir, location ) |> move, numInfections )

        _ ->
            ( Dict.insert location Weakened nodes, turnLeft ( dir, location ) |> move, numInfections )
...
parseLine : Int -> ( Int, String ) -> Nodes -> Nodes
parseLine numRows ( row, textLine ) nodes =
    let
        numCols =
            String.length textLine
    in
    textLine
        |> String.toList
        |> List.indexedMap (\a b -> ( a, b ))
