...
firstStepTest board src tgt =
    let
        b =
            board
                |> gSet src (Occupied (Unit 0 Elf src 0 0))
                |> gSet tgt (Occupied (Unit 0 Goblin tgt 0 0))

        ( d, row, col ) =
            shortestPathStep src tgt b |> Maybe.withDefault ( 0, 0, 0 )
    in
    gSet ( row, col ) (Distance d) b |> displayBoard
```
...
firstStepTest7 =
    let
        blockedBoard =
            testEmpty |> parseBoard |> gSet ( 3, 4 ) Wall
    in
...
firstStepTest8 =
    let
        blockedBoard =
            testEmpty |> parseBoard |> gSet ( 3, 4 ) Wall |> gSet ( 4, 3 ) Wall
    in
    firstStepTest blockedBoard ( 4, 4 ) ( 3, 1 )
...
firstStepTest9 =
    let
        blockedBoard =
            testEmpty |> parseBoard |> gSet ( 3, 4 ) Wall |> gSet ( 4, 3 ) Wall |> gSet ( 5, 4 ) Wall
    in
    firstStepTest blockedBoard ( 4, 4 ) ( 3, 1 )
```
...
firstStepTest10 =
    let
        blockedBoard =
            testEmpty |> parseBoard |> gSet ( 3, 4 ) Wall |> gSet ( 4, 3 ) Wall |> gSet ( 5, 4 ) Wall |> gSet ( 4, 5 ) Wall
    in
    firstStepTest blockedBoard ( 4, 4 ) ( 3, 1 )
```
    ...
    let
        ...
        units row =
            List.foldl
                ...
                ""
                (gGetRow row board |> Maybe.withDefault [])
        ...
        db =
            gMap cellToText board
    in
    List.foldl
        (\row -> flip (++) ("\n    " :: [ gGetRow row db |> Maybe.withDefault [] |> String.concat, units row ]))
        [ "    " ]
        (List.range 0 (gRowCount board - 1))
```
