{- To keep the Elves busy, Santa has them deliver some presents by hand, door-to-door.
    He sends them down a street with infinite houses numbered sequentially: 1, 2,
    3, 4, 5, and so on.
...
part1 : String -> Int
part1 input =
    {- TODO: Could optimise by finding the lower bound (rather than 1) as the
       start number. Robin's inequality (assuming house number is at least 5040)
       provides this lower bound: https://en.wikipedia.org/wiki/Divisor_function#Growth_rate
    -}
    firstHouse 1 (toInt input)


part2 : String -> Int
part2 input =
    firstHousePt2 1 (toInt input)


firstHouse : Int -> Int -> Int
