...
The entire rectangle of fabric can be represented by an array of integers (using `Grid` from [aoc.md](aoc.md)):
...
And therefore, the sum of the values for such a rectangle should equal its width x height:
...
part2 =
    ...
    puzzleInput
        |> parse
        |> List.foldl (checkRect filledGrid) 0
```
