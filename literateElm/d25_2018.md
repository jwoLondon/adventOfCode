...
Parsing is simply a case of splitting input text by commas and desctructuring into `Point` records.
...
parse =
    ...
    List.map (String.split "," >> toPoint)
        ...
        >> List.sortBy (\p -> ( p.a, p.b ))
        >> List.sortBy (\p -> ( p.c, p.d ))
```
...
addToClusters p clusters =
    ...
    in
    if nearCls == [] then
        -- Point is unconnected with any existing clusters so add a new one
        [ p ] :: clusters

    else
        -- Point is in at least one cluster so merge them
...
part1 =
    puzzleInput
        |> parse
        |> List.foldl (\p clusters -> addToClusters p clusters) []
        |> List.length
```

## Reflection

A nice easy one for Christmas morning, more in the tradition of AoCs in previous years. My inital thoughts on approaching this were to test for all pairwise distances as each point is added and then re-compute clusters, but that could become prohibitively expensive as the number of points increased. I think, but am not certain, that sorting the input points first improves efficiency as we are more likely to accumulate large clusters first.
