...
```elm {l}
search : Cave -> Deque EquippedLocation -> Times -> Times
...
part2 =
    ...
    -- search cave tgt (Deque.singleton ( 0, 0, Torch )) Dict.empty
...
My first attempt at part two was unecessarily complicated in trying to store multiple graphs for different equipment types. This soon led to unweildy code which I abandoned. I was pleased to be able recognise that the equipment choices could be represented as a simple graph of (row,col,equipment) nodes. However the building of this graph is slow. This is because new shortest times can cascade back through the graph as neighbouring values are updated.
