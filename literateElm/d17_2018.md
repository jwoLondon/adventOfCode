...
```elm {l}
addWater : Location -> Deque Location -> Slice -> ( Slice, Deque Location )
    ...
    if row == gRowCount slice - 1 then
        ...
        if below == Sand || below == MovingWater then
            ...
            if blockedLeft then
                ...
                addWater ( row + 1, r ) (Deque.pushBack ( row + 1, l ) todo) (List.foldl (\c sl -> gSet ( row, c ) MovingWater sl) slice (List.range l r))
...
```elm {l}
simulate : ( Slice, Deque Location ) -> Slice
simulate ( slice, source ) =
    let
        ( maybeLocation, todo ) =
            Deque.popFront source
    in
    ...
    let
        ...
        countWater =
            gToLists
                ...
                >> List.length
    in
    simulate ( slice, Deque.singleton (springOut slice) ) |> countWater
```
...
part2 =
    let
        slice =
            parse puzzleInput
    in
    simulate ( slice, Deque.singleton (springOut slice) )
        |> gToList
