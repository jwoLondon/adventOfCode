...
The question provides us with a test case to check things work as expectedly (4 IDs with two repeated letters and 3 IDs with three repeated letters):
...
Now we need to be able to identify whether a pair of strings differ by only one character.
...
To answer the question we will need to compare each string with every other one (from the set of IDs containing two or three repreated letters). We can use `combinations` from [aoc utilities](aoc.md) choosing 2 from the list to find all pairwise combinations from any given set of strings.
        ...
        diff pair =
            case pair of
                ...
                _ ->
                    Nothing
    in
    combinations 2 >> List.filterMap diff
```
