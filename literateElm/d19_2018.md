...
```elm {l}
parseDay19 : List String -> Dict Int (Registers -> Registers)
parseDay19 =
    let
        toOp instr =
            case instr of
...
part1 =
    let
        program =
            Program 5 (init [ 0, 0, 0, 0, 0, 0 ]) (parseDay19 puzzleInputDay19) 0
    in
    run program
        |> .registers
        |> get 0
...
| Position | Instruction | Param A | Param B | Param C | action             |
| -------: | ----------- | ------: | ------: | ------: | ------------------ |
