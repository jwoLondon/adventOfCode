...
Count the number of _valid_ passports - those that have all required fields. Treat cid as optional. **In your batch file, how many passports are valid?**
...
isValid pRec =
    Dict.member "byr" pRec
        && Dict.member "iyr" pRec
        && Dict.member "eyr" pRec
        && Dict.member "hgt" pRec
        && Dict.member "hcl" pRec
        && Dict.member "ecl" pRec
        && Dict.member "pid" pRec
```
...
```elm {l}
validateByr : PRecord -> Bool
validateByr pRec =
    case Dict.get "byr" pRec of
        Just date ->
            date >= "1920" && date <= "2002"
...
```elm {l}
validateIyr : PRecord -> Bool
validateIyr pRec =
    case Dict.get "iyr" pRec of
        Just date ->
            date >= "2010" && date <= "2020"
...
```elm {l}
validateEyr : PRecord -> Bool
validateEyr pRec =
    case Dict.get "eyr" pRec of
        Just date ->
            date >= "2020" && date <= "2030"
...
```elm {l}
validateHcl : PRecord -> Bool
validateHcl pRec =
    case Dict.get "hcl" pRec of
        Just hcl ->
            List.length (AOC.submatches "^#([a-f|0-9]{6})$" hcl) == 1
...
```elm {l}
validateEcl : PRecord -> Bool
validateEcl pRec =
    case Dict.get "ecl" pRec of
        Just ecl ->
            List.length (AOC.submatches "^(amb|blu|brn|gry|grn|hzl|oth)$" ecl) == 1
...
```elm {l}
validatePid : PRecord -> Bool
validatePid pRec =
    case Dict.get "pid" pRec of
        Just pid ->
            List.length (AOC.submatches "^([0-9]{9})$" pid) == 1
