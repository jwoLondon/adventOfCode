...
Each seat code is just a number represented in binary with 'F' and 'L' representing 0 and 'B' and 'R' representing 1. We can therefore just convert to binary, extracting the left 8 bits for rows and the right 3 bits for columns.
...
seatLocation s =
    let
        binStr =
            AOC.replace "[FL]" "0" >> AOC.replace "[BR]" "1"
    in
    ( AOC.binaryToDec (String.left 7 (binStr s)), AOC.binaryToDec (String.right 3 (binStr s)) )
```
