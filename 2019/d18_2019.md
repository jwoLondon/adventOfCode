...
From this it is easy to create a sub-graph that contains all connected keys given a set of carried keys.
...
Now we have a navigable graph representing the maze with any given collection of carried keys, we can more efficiently determine any key's uncollected adjacent keys and their number of steps away:

```elm {l}
uncollectedAdjacentKeys : Char -> List Char -> Tunnels -> List ( Char, Int )
uncollectedAdjacentKeys fromKey carried tunnels =
    let
        gAccess =
            accessibleTunnels (Set.fromList carried) tunnels
        ...
        search todo visited adj =
            case todo of
                k1 :: tl ->
                    let
                        ( newTodo, newAdj ) =
                            Set.partition (\k -> List.member k carried)
                                (Graph.outgoing k1 gAccess |> Set.filter (\k -> (not << Set.member k) visited))
                                |> Tuple.mapSecond (AOC.flip Set.diff (Set.fromList (List.map Tuple.first adj)))
                    in
                    search (Set.toList newTodo ++ tl)
                        (Set.insert k1 visited)
                        ((Set.toList newAdj |> List.map keyDist) ++ adj)

                [] ->
                    adj
    in
    search [ fromKey ] Set.empty []
```
                    ...
                    let
                        ...
                        addPath ( k, d ) ( td, ps ) =
                            ...
                            case Dict.get ( id, k ) ps of
                                Nothing ->
                                    ( ( k :: carried, newDist ) :: td, Dict.insert ( id, k ) newDist ps )
                        ...
                        ( newTodo, newPaths ) =
                            List.foldl addPath ( tl, paths ) toKeys
                    in
                    search newTodo newPaths
    in
    search [ ( [ '@' ], 0 ) ] Dict.empty
```
...
This puzzle was too much for me in my limited time in December so I revisited it later in the the year. Even with the lack of time pressure, this was tough and it took me several attempts to land on a workable solution. This was the puzzle in the since the start of AoC that took me the longest to code a workable solution.
