...
type alias PortalLookup =
    Dict String (List GridLocation)


isLetter : Grid Char -> GridLocation -> Bool
isLetter g loc =
    case gGet loc g of
        Just chr ->
            chr >= 'A' && chr <= 'Z'
...
addPortal : Grid Char -> GridLocation -> PortalLookup -> PortalLookup
addPortal g ( row, col ) dict =
    ...
    if isLetter g ( row, col ) && isLetter g ( row, col + 1 ) then
        let
            name =
                [ gGet ( row, col ) g, gGet ( row, col + 1 ) g ]
        ...
        let
            name =
                [ gGet ( row, col ) g, gGet ( row + 1, col ) g ]
                    |> List.filterMap identity
                    |> String.fromList
...
portals =
    List.foldl (addPortal maze)
        Dict.empty
        (gridLocations ( 0, 0 ) ( gRowCount maze, gColCount maze ))
```
...
```elm {l}
portalName : Grid Char -> GridLocation -> String
portalName g ( row, col ) =
    if isLetter g ( row, col ) && isLetter g ( row, col + 1 ) then
        ...
        [ gGet ( row, col - 1 ) g, gGet ( row, col ) g ]
            |> List.filterMap identity
            |> String.fromList
        ...
        [ gGet ( row - 1, col ) g, gGet ( row, col ) g ]
            |> List.filterMap identity
            |> String.fromList
...
```elm {l}
teleport : String -> GridLocation -> GridLocation
teleport label loc =
...
```elm {l}
findExit : GridLocation -> List GridLocation -> Int -> Int
findExit ( row, col ) visited steps =
    let
        stepsToExit ( r0, c0 ) ( r1, c1 ) st =
            if gGet ( r1, c1 ) maze == Just '.' && not (List.member ( r1, c1 ) visited) then
...
<!-- ```elm {l}
findExit : List GridLocation -> Deque GridLocation -> Int -> Int
findExit visited toVisit steps =
    case Deque.popFront toVisit of
        ( Nothing, remaining ) ->
            steps

        ( Just ( row, col ), remaining ) ->
            -- TODO: Check new location and move, possibly via portal, accumulating steps
            if ( row, col ) == exitLocation then
                steps
