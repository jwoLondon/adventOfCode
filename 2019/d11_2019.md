...
```elm {l}
move : Int -> ( GridLocation, Direction ) -> ( GridLocation, Direction )
move tNum ( ( x, y ), ( dx, dy ) ) =
    let
        ( newDx, newDy ) =
            case toTurn tNum of
...
toGrid locations =
    let
        ...
        empty =
            gInit (1 + maxY - minY) (1 + maxX - minX) '▒'

        grid =
            List.foldl (\( x, y ) g -> gSet ( y - minY, x - minX ) '█' g) empty locations

        lines =
            grid
                |> gToList
                |> groupsOf (gColCount grid)
                |> List.map ((::) '\n')
                |> List.concat
    in
    grid
...
On the face of it, this should have been a fairly simple puzzle to solve. I felt I knew what needed to be done and by building up a dictionary of painted panels, this should have yielded the correct answer. But frustratingly I was getting an incorrect count for part 1 (about a third of the correct answer).
