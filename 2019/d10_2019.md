...
```elm {l}
asteroidLocations : Grid Char -> List GridLocation
asteroidLocations =
    gMapWithLocation
        (\loc z ->
            if z == '#' then
...
```elm {l}
otherAsteroids : GridLocation -> Grid Char -> List GridLocation
...
```elm {l}
blocked : Int -> Int -> GridLocation -> GridLocation -> List GridLocation
blocked nRows nCols ( row0, col0 ) ( rowB, colB ) =
    if row0 == rowB && col0 == colB then
        ...
        let
            ...
            hcf =
                highestCommonFactor (rowB - row0) (colB - col0)
...
```elm {l}
directView : GridLocation -> Grid Char -> Grid Char
...
```elm {l}
orderedLocations : GridLocation -> Grid Char -> List GridLocation
