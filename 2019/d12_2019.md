...
Updating the positions and velocities of each moon is straightforward if we are prepared to do so iteratively. Being an 4-body problem, this makes sense as an approach rather than attempting to solve analytically.
...
tick ( positions, velocities ) =
    ...
    in
    ( zip3 ( newPs pxs vxs, newPs pys vys, newPs pzs vzs )
    , zip3 ( newVs pxs vxs, newVs pys vys, newVs pzs vzs )
    )
```
...
updateVelocity pComponents vComponents =
    let
        newVComponents ps vs =
            case List.map gravity (pairwiseCombinations pComponents) of
...
part2 =
    let
        ...
        xCycle =
            sequenceCycle 0 ( pxs, [ 0, 0, 0, 0 ] ) componentTick
                |> Maybe.withDefault ( 0, 0 )
                |> Tuple.first

        yCycle =
            sequenceCycle 0 ( pys, [ 0, 0, 0, 0 ] ) componentTick
                |> Maybe.withDefault ( 0, 0 )
                |> Tuple.first

        zCycle =
            sequenceCycle 0 ( pzs, [ 0, 0, 0, 0 ] ) componentTick
                |> Maybe.withDefault ( 0, 0 )
                |> Tuple.first
    in
    List.foldl lowestCommonMultiple 1 [ xCycle, yCycle, zCycle ]
```
...
While obvious a cycle detecting algorithm would be necessary it took me a little while to realise that I could find cycles the x, y and z components of the positions and velocities completely independently and then just find the lowest common multiple. This should have been more obvious to me given that I calculated the positions/velocities for the three components separately.

Although several previous puzzles have required the detection of cycles, these have typically been with a period or offset sufficiently small to allow naive detection. For this exercise I had to implement `sequenceCycle` (added to [aoc.md](../aoc.md)).
